...
import PesananTable from "@/components/pesanan/PesananTable";
import SummaryBulananCard from "@/components/pesanan/SummaryCard";
...
const [rangeData, setRangeData] = useState({
  ...
  const fetchPesanan = async (page = 1, customFilters = filters) => {
    try {
      setLoading(true)

      const response = await apiFetch("/api/all/pesanan", {
        method: "POST",
        body: JSON.stringify({
          page,
          perPage, 
          search,  
          fullname: customFilters.fullname || "",
          kelas: customFilters.kelas || "",
          id_produk: customFilters.produkId || 0,
          min_harga: customFilters.hargaMin || 0,
          max_harga: customFilters.hargaMax || 0,
          start_date: customFilters.tanggalAwal || null,
          end_date: customFilters.tanggalAkhir || null,
          min_terjual: customFilters.minTerjual || 0,
          max_terjual: customFilters.maxTerjual || 0
      ...
      if (response.success) {
        ...
        setRangeData(response.range) // ⬅ penting
      }
